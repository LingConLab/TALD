---
title: "Adpositions"
author: "Tatiana Philippova, Polina Nasledskova"
...
gloss_example(transliteration = "**Im** **ěnker-oǰ** **šnorhiv** darj-i ē gal-is ew ěndun-um k’ristoneakan havatk’-ě",
              glosses = "my friend-GEN.DAT thanks conversion-DAT he_is come-PTCP.PRS.3SG CONJ adopt-PTCP.PRS Christian belief.NOM-the",
...
gloss_example(transliteration = "Xač’atryan-i gorc-er-i-n canot’ac’-el ē **šnorhiv** **ir** **usuc’č’-i** Nat’an Milšteyn-i um mot usan-el ē 10 tari",
              glosses = "Xachatryan-DAT work-PL-DAT-the get_acquainted-PTCP.PF he_is thanks his teacher-GEN.DAT Nathan Milstein-GEN.DAT REL.DAT at learn-PTCP.PF he_is ten year.NOM",
...
a.
```{r}
gloss_example(transliteration = "**k’o** **handep** aysteł mec hargank’ ka",
...
|------------|-----------|-----------|------------|
| Hinuq  | Tsezic   | *geł* (ESS), *geł-es* (ABL1), *geł-zo* (ABL2), *geł-er* (LAT), *geł-edo* (DIR)   | @forker2013: 379 |
| Mehweb | Dargic   | *ʔu* (LAT), *ʔu-b* (ESS), *ʔu-b-adal* (EL)   | @magometov1982: 126 |
| Bagvalal | Andic | *hiƛ’i* (ESS), *hiƛ’i-sː* (EL), *hiƛ’i-sːini* (TRLAT) | @sosenskaya2001: 170 |
| Lezgian | Lezgic | *k’anik* (ESS), *k’anikaj* (EL) | @haspelmath1993: 219-220 |
| Chechen | Nakh | *k’el*  (ESS/LAT), *k’eləħ* (ESS), *k’elərə* (EL), *k’elxuʊlə* (TRLAT),  *k’elħaː* (DIR) | @aliroev1999: 79-80 |
...
a.
```{r}
gloss_example(transliteration = "de č’agu goł q’ede, **de-de** **purho**=no nox-no",
...
gloss_example(transliteration = "meži	di-de purho-s b-uɬi-yom",
              glosses = "you.PL I.OBL-ALOC next-ABL1 HPL-gp.out-PROH",
...
gloss_example(transliteration = "**getn-i** **tak-ov** mi mec get eː hos-um",
              glosses = "ground-DAT under-INS INDEF big river.NOM it.is flow-PTCP.PRS",
...
Turning now to agreement morphemes on postpositions, we note first that it is attested in languages of all branches of East Caucasian, except for Nakh and Khinalug. Secondly, in every language that has it, it is not pervasive: it is always a minority of postpositions that agree. Agreement slots may be prefixal, suffixal or (rarely) infixal, as shown in (@five), (@six) and (@seven), respectively. 
...
```{r}
gloss_example(transliteration = "školal-da ask’o-**w** jasał was w-uχana",
...
```{r, results='asis'}
gloss_example(transliteration = "zari gǝzet o‹**b**›kłni arχul-ma-k e‹b›q’en",
              glosses = "1SG.ERG newspaper.SG.ABS(III) read<III.SG>.PFV the.middle-IN-LAT up.to<III.SG>",
...
gloss_example(transliteration = "**qovluǧ-un** **ara-sı-n-dan** bir vərəqə çıx-ar-ıb",
              glosses = "folder-GEN interval-3.POSS-EP-ABL one sheet go.out-CAUS-ANT",
              free_translation = "having put out one sheet from the folder…")
```

In (@nine), the possessive marker on the postposition *arasından* corresponds to the person of its dependent (‘folder’). Note, however, that experts typically do not consider such lexemes as adpositions, attributing them to nouns (e.g., @shiraliyevsevortyan1971: 165); our dataset, therefore, does not include them either. Adpositions that agree with the absolutive (or nominative, for that matter) are not attested in Turkic languages.
...
gloss_example(transliteration = "**inj** **het** mišt hayeren eː xos-um",
              glosses = "I.DAT with always Armenian he.is speak-PTCP.PRS",
...
gloss_example(transliteration = "**het-s** mišt hayeren eː xos-um",
              glosses = "with-my always Armenian he.is speak-PTCP.PRS",
              free_translation = "He always speaks Armenian with me.",
...
For a more detailed discussion of agreeing adpositions, see chapter Agreement of adpositions with the nominative (in preparation).
...
gloss_example(transliteration = "**t'exkaa** **chuhwanahw** t'um joall",
              glosses = "bone.DAT inside marrow II.be.contained.PRS",
...
gloss_example(transliteration = "p’ap’ul pap-n as-um eː or girk’-e erbek’ **tn-ic’** **durs** č’-i han-el",
              glosses = "P’ap’ul grandpa.NOM-the say-PTCP.PRS he.is CONJ book.NOM-the never house-ABL out NEG-he.is take-PTCP.PF",
...
gloss_example(transliteration = "ov č’-i irakana-c’n-i petut’y-an cragir-e na **durs** kmn-a ayd gorčent’ac’-ic’",
              glosses = "who NEG-he.is fulfill-CAUS-COND.FUT.3SG state-DAT program.NOM-the he.NOM outside stay-COND.FUT.3SG that process-ABL",
...
### Case assignment
The set of cases that are assigned by postpositions even within one East Caucasian language is pretty large and includes both syntactic (abstract) cases and spatial cases[^2]. This may be illustrated by the case assignment properties of semantically similar postpositions from languages of different branches of East Caucasian.

(@thirteen) Chechen (Nakh) [@yakovlev1940: 143]
```{r}
gloss_example(transliteration = "**gɔʊr-a-na** / **gɔʊr** **t’e** ʡyrre so xiːrə",
              glosses = "horse-OBL-DAT {/} horse.ABS on in.the.morning I sit",
              free_translation = "I sat on a horse in the morning.")
```
(@fourteen) Bagvalal (Andic) [@sosenskaya2001: 406]
```{r}
gloss_example(transliteration = "hinc’-ib-a-ła č’ihi",
              glosses = "stone-PL-OBL.PL-SUP on",
              free_translation = "on stones")
```
(@fifteen) Tsakhur (Lezgic) [@sosenskaya1999b: 127]
```{r, results='asis'}
gloss_example(transliteration = "sa jiʁ-ɨ-lʲ ǯamaʔaIt-ɨ-k’le **χāI-ni** **aq’ʷalʲ** c’e=t=ta wertalʲot ɢaǯe",
              glosses = "one day-OBL-SUP people-OBL-AFF sky-OBL on first=IV=ADV.IV helicopter.IV IV.see.PFV",
              free_translation = "Once the people saw a helicopter in the sky for the first time.")
```

There is no clear correlation between the postposition’s meaning (spatial or not) and the type of case it assigns (spatial or not): there are abstract postpositions that assign spatial cases and spatial postpositions requiring an abstract case, as can be seen from the examples below.
...
(@eighteen)	Sanzhi Dargwa [@forker2020: 156]
a.	
```{r}
gloss_example(transliteration = "nuša šːi-la tːura ag-ur=da",
              glosses = "1.PL village-GEN outside go.PFV-PRET=I",
...
(@nineteen)	Tsez [@polinsky2015: 177]
a.
```{r, results='asis'}
gloss_example(transliteration = "eli mariyat-**qo-r** ƛexuzaƛ’or c’alduqan-za-qo-r xabaryay-nč’i",
              glosses = "1PL.ABS Mariyat-**POSS-LAT** except student-OS.PL-POSS-LAT talk-PST.W.NEG",
              free_translation = "We did not talk to the students except Mariyat.")
```
b.
```{r}
gloss_example(transliteration = "eli ɣudes ħalt’-o-ł-xor b-ik’i-x ħat’an-**ƛ’o** ƛexuzaƛ’or",
              glosses = "1.PL.ABS daily work-OS-CONT-VERS 1.PL-go-PRS Sunday-**SUP.ESS** except",
...
gloss_example(transliteration = "xõx-ǧur rišo/gišo/tišo",
              glosses = "tree-COM upwards/downwards/horizontal.direction",
              free_translation = "(upwards/downwards) along the tree")
```
b.
```{r}
gloss_example(transliteration = "xõx-i-i rišo/gišo/tišo",
              glosses = "tree-OBL-DAT upwards/downwards/horizontal.direction",
...
gloss_example(transliteration = "ħa **c’en** **t’eħa** berkat=ʔa xurgdaːc",
              glosses = "2SG.GEN house.GEN after abundance=and be.FUT.IV.NEG",
...
gloss_example(transliteration = "so čy-iːqːaːča legjolaž jar yz **suːona** **t’eħa**",
              glosses = "1SG in-jump.PTCP.OBL die.FUT.II.SIM II.be.PST 3SG 1SG.DAT after",
              free_translation = "She was eager (‘dying’) to go anywhere I went (‘jumped, rushed’).")
```

In (@twentyone), the same postposition *t’eħa* ‘after’ requires the genitive on its dependent nouns and the dative on its dependent pronouns. A similar, though more systematic, phenomenon is attested in Mehweb: while postpositions can govern various forms of nouns (dative, genitive, comitative, absolutive), all pronominal dependents of postpositions take the dative case form [@magometov1982: 127]. 
