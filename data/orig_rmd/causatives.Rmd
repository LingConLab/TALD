...
file_name <- "causatives_bib.tsv"
read_tsv(file_name, progress = FALSE) %>% 
  bib2df::df2bib(file_name %>% str_remove_all("[_\\.]tsv") %>%  str_replace("_bib$", "\\.bib"))
```
...
## List of glosses
```{r, echo=FALSE}
added_glosses <- data.frame(gloss = c("NOTN"),
                               definition_en = c("non-neuter pronominal suffix"))
make_gloss_list(added_glosses)
```
