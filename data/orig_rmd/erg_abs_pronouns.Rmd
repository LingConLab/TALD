...
This possibility is exemplified by Botlikh, an Andic language which distinguishes between ergative and absolutive only in its 1SG pronoun and lacks this distinction in all other pronouns. For the `r add_gloss("1SG")` pronoun, *den* is used in the absolutive and *iškur* in the ergative, while all the other pronouns take the same form when used in the two functions: *min* (`r add_gloss("2SG")`), *iƛi* (`r add_gloss("1PL.INCL")`), *išːi* (`r add_gloss("1PL.EXCL")`) and *bišti* (`r add_gloss("2PL")`) [@alexeyevverheesdraft2020: 12]; see [Table 4](#t4).

##### **Table 4.** Ergative and absolutive in personal pronouns: Botlikh (< Andic) {#t4 -}

|                      | `r add_gloss("1SG")` | `r add_gloss("2SG")` | `r add_gloss("1PL.INCL")` | `r add_gloss("1PL.EXCL")` | `r add_gloss("2PL")` |
|:--------------------:|:--------------------:|:--------------------:|:-------------------------:|:-------------------------:|:--------------------:|
...
This possibility is observed in Khinalug, where only `r add_gloss("1SGL")` and `r add_gloss("2SG")` pronouns differ with regard to the ergative-absolutive function: for the `r add_gloss("1SG")` pronoun, *zɯ* is used in the absolutive and *jæ* in the ergative, for the `r add_gloss("2SG")` pronoun, *vɯ* is used in the absolutive and *va* in the ergative. `r add_gloss("1PL.INCL")`, `r add_gloss("1PL.EXCL")` and `r add_gloss("2PL")` pronouns (that is, all plural pronouns) use the same form in both functions [@kibriketal1972: 73]; see [Table 5](#t5).
