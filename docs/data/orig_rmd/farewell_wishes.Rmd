...
Even within the same language community (such as a village), several farewell formulae can be used. For example, as found within a field study in Rutul villages, in the village of Kufa three speakers produced three different formulae:

(@kufa4) Kufa Rutul (field data by Maxim Melenchenko)
```{r}
...
(@kufa5) Kufa Rutul (field data by Maxim Melenchenko)
```{r}
...
(@kufa6) Kufa Rutul (field data by Maxim Melenchenko)
```{r}
...
Still, there usually exists one main formula which is the most frequent. In the above-mentioned village of Kufa all three speakers produced the formula in (@kufa4), while the other two formulae came up only once. In the other eleven Rutul villages which were surveyed within the same field research, the result was similar - the formula in (@kufa4) is by far the most common one. However, since most of the data analyzed in this chapter were collected from dictionaries, in most cases I did not have the opportunity to check which formula is the most common. Therefore, for each language I listed and mapped all formulae which were mentioned as expressing farewell meaning, and treated them equally.
...
gloss_example(transliteration = "huni 	bic’i		r-aq-ob",
              glosses = "road 	straight		5-happen-OPT",
              free_translation = "May the road be straight!")
```
...
(@aghul14) Aghul (Huppuq’ dialect) (Solmaz Merdanova, p.c.)
```{r}
...
(@tsez16) Tsez  [@khalilov1999: 109]
```{r}
gloss_example(transliteration = "huni 	r-egir-o-ƛ",
              glosses = "road 	4-send/let-IMP-OPT",
