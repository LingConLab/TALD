...
feature_dataset <- read_tsv("../orig_table/demonstratives.tsv")
...
```{r}
all_genpoints %>% 
  filter(!is.na(all_genpoints$value3)) %>% 
  add_count(value3) %>% 
...
```{r}
alldata_clean %>% 
  filter(!is.na(alldata_clean$value3)) ->
...
```{r}
all_genpoints %>% 
  filter(!is.na(all_genpoints$value4)) %>% 
  add_count(value4) %>% 
...
```{r}
alldata_clean %>% 
  filter(!is.na(alldata_clean$value4)) ->
  alldata_clean_filtered
...
```{r}
all_genpoints %>% 
  filter(!is.na(all_genpoints$value5)) %>% 
  add_count(value5) %>% 
  mutate(value5 = str_c(value5, ' (', n, ')')) ->
  all_genpoints_filtered

map.feature(all_genpoints_filtered$lang4map,
            latitude = all_genpoints_filtered$lat,
            longitude = all_genpoints_filtered$lon,
            features = all_genpoints_filtered$value5,
            title = all_genpoints_filtered$value5_name[1],
            color = 'magma',
            stroke.features = 'a',
            stroke.color = 'black',
            stroke.radius = 1,
...
```{r}
alldata_clean %>% 
  filter(!is.na(alldata_clean$value5)) ->
  alldata_clean_filtered

map.feature(alldata_clean_filtered$lang4map,
            latitude = alldata_clean_filtered$lat, 
            longitude = alldata_clean_filtered$lon,
            features = alldata_clean_filtered$value5,
            color = 'magma',
            stroke.features = 'a',
            stroke.color = 'black',
            stroke.radius = 1,
            stroke.legend = FALSE,
            tile = 'Stamen.TonerLite',
            title = alldata_clean_filtered$value5_name[1],
            legend = TRUE,
            legend.position = 'bottomleft', 
            label = alldata_clean_filtered$lang,
            zoom.control = TRUE,
...
[Download](https://raw.githubusercontent.com/LingConLab/TALD/master/data/orig_table/demonstratives.tsv) the whole dataset and [its description](https://raw.githubusercontent.com/LingConLab/TALD/master/data/md_description/demonstratives.md).
