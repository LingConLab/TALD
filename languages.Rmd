...
intermediate |> 
  mutate(insert_a_raw = if_else(lag(value) == "Avar-Andic" & value != "Avar", "Andic", NA),
         insert_a_raw_id = 1:n()) |> 
  na.omit() |> 
  pull(insert_a_raw_id) ->
  insert_a_raw_ids

map(seq_along(insert_a_raw_ids), function(i){
  if(i == 1){
    bind_rows(intermediate[1:(insert_a_raw_ids[i]-1),], tibble(cols = "insert", value = "Andic"))
  } else {
    bind_rows(intermediate[(insert_a_raw_ids[i-1]):(insert_a_raw_ids[i]-1),], tibble(cols = "insert", value = "Andic"))
  }
}) |> 
  list_rbind() |> 
  bind_rows(intermediate[(insert_a_raw_ids[length(insert_a_raw_ids)]):nrow(intermediate),]) |> 
  fill(id) |> 
  group_by(id) |> 
